...
**Mechanism**:
- Uses sequence numbers modulo-2^m (m is number of bits in sequence field)
...
**Mechanism**:
- Uses sequence numbers modulo-2^m
- Sender window size: 2^(m-1)
...
1. Need 4 parity bits (positions 1, 2, 4, 8) as 2^3 < 11 <= 2^4
...
**Key Requirement**:
- Minimum frame time >= 2 x maximum propagation time
...
Data = (d1.c1 + d2.c2 + d3.c3 + d4.c4) x c1 = 4 x d1
...
3. Using CRC with generator polynomial x^3 + x + 1, compute the CRC for data 10110.
